...
/ all tables in the top level namespace (`.) become publishable
\l tick/u.q
.u.init[];
...
.ringBuffer.write:{[t;r;i] @[t;(i mod count t)+til 1;:;r];}
...
.stream.wavesGen:{ res: enlist `time`r`o`y`g`b`p!(.z.p,{(x%6)+.math.sineWave[.z.p;1.4;1e-10f;x] - 0.4} each til 6); .ringBuffer.write[`.stream.waves;res;.stream.i+:1]; res }
